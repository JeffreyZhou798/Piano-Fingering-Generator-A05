...
async function parallelTrain(
  ...
  const workerPromises = workerConfigs.map((config, index) => {
    return new Promise<Map<string, number>>((resolve, reject) => {
      ...
      const workerCode = `
        ...
        self.onmessage = async (e) => {
          ...
          try {
            ...
            const qTableArray = Array.from(qTable.entries());
      ...
      // 由于Next.js的限制，我们暂时使用单线程实现
      // TODO: 在生产环境中使用真正的Web Workers
      console.warn('Web Workers not fully supported in this build, using single-threaded fallback');
